...
HT.Hexagon = function(id, x, y) {
	...
	this.svgPath = 'M';// Relative path base
	...
	if(HT.Hexagon.Static.ORIENTATION == HT.Hexagon.Orientation.Normal) {
		...
		for (var i=0; i< this.Points.length; i++){
			this.svgPath += this.Points[i]['X']+','+this.Points[i]['Y']+'L';
		}
		this.svgPath = this.svgPath.replace(/L$/, 'z');
...
/*	
	...
	this.r = R2.path( this.Path).attr(attr);
	this.r.color = '#00a2bf';
	//this.path = R.path( this.Points);
	//this.attr = attr;
	//this.color = '#ffffff';
	//console.log( this);
	*/
